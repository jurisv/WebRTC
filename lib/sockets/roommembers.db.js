    ...
    var firebase = require('./../firebase.db.js')(nconf),
        ...
        roommembers = {
            ...
            create: function(config,callback){
                ...
                if(records instanceof Array){
                    records.forEach(function(item) {
                        firebase.child('roommembers/' + config.params.room + '/' + item.id).update(item);
                        callback(null,[item.records]);
                    });
                }else{
                    firebase.child('roommembers/' + config.params.room + '/' + config.records.id).update(config.records);
            ...
            delete: function(config,callback){
                ...
                if(records instanceof Array){
                    records.forEach(function(item) {
                        firebase.child('roommembers/'  + config.params.room + '/' + item.id).remove();
                        callback(null,item);
                    });
                }else{
                    firebase.child('roommembers/'  + config.params.room + '/' + config.records.id).remove();
