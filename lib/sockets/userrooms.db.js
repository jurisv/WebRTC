    ...
    var firebase = require('./../firebase.db.js')(nconf),
        ...
        rooms = {
                ...
                if(records instanceof Array){
                    records.forEach(function(item) {
                        opentok.getOpenTokSessionId(function(err,sessionId){
                            ...
                            firebase.child('userrooms/' + item.id).update(item, function(err){
                                if(err){
                                    callback(err,null);
                                }else{
                    ...
                    opentok.getOpenTokSessionId(function(err,sessionId){
                        ...
                        firebase.child('userrooms/' + config.records.id).update(config.records, function(err){
                            if(err){
                                callback(err,null);
                            }else{
            ...
            delete: function(config,callback){
                ...
                if(records instanceof Array){
                    records.forEach(function(item) {
                        firebase.child('userrooms/' + item.id).remove(function(err){
                            if(err){
                                callback(err,null);
                            }else{
                                callback(null,item);
                    ...
                    if (!!config.records.id) {
                        firebase.child('userrooms/' + config.records.id).remove(function (err) {
                            if (err) {
                                callback(err, null);
                            } else {
