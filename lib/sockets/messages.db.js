       ...
       messages = {
            ...
            bindChanges: function(io, config, socket){
                ...
                if(!io.messages[room]) {
                    ...
                    io.messages[room].on('child_added', function (childSnapshot, prevChildName) {
            ...
            create: function(config,callback){
                ...
                if(records instanceof Array){
                    records.forEach(function(item) {
                        firebase.child('messages/' + config.params.room + '/' + item.id).update(item, function(err){
                            if(err){
                                callback(err,null);
                            }else{
                    ...
                    firebase.child('messages/' + config.params.room + '/' + config.records.id).update(config.records, function(err){
                        if(err){
                            callback(err,null);
                        }else{
            ...
            delete: function(config,callback){
                ...
                if(records instanceof Array){
                    records.forEach(function(item) {
                        if (!!config.params.room && !!item.id) {
                            firebase.child('messages/' + config.params.room + '/' + item.id).remove(function (err) {
                                if (err) {
                                    callback(err, null);
                                } else {
                                    callback(null, item);
                    ...
                    if (!!config.params.room && !!config.records.id) {
                        firebase.child('messages/' + config.params.room + '/' + config.records.id).remove(function (err) {
                            if (err) {
                                callback(err, null);
                            } else {
