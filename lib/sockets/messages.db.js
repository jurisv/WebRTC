var firebase = require('firebase'), //npm install firebase
    ...
    tokenGenerator = new firebaseTokenGenerator(app.config.get('adminsettings').serviceprovider.firebase.SecretKey || ''),  //secret key set in environmnet variable : do not hardcode
...
var messages = {
    ...
    _baseRef: new firebase( app.config.get('adminsettings').serviceprovider.firebase.Url ),
    _messages: [],
        ...
        if(!io._messagesRef) {
           ...
           /* io._messagesRef.on('child_added', function (childSnapshot, prevChildName) {
                if (childSnapshot.val() && childSnapshot.val() != undefined) {
                    ...
                    app.io.of('/messages').emit('child_added', app.wrapresponse(data));
                }
            });
            */
            ...
            io._messagesRef.on('child_removed', function (childSnapshot, prevChildName) {
                if (childSnapshot.val() && childSnapshot.val() != undefined) {
                    ...
                    app.io.of('/messages').emit('child_removed', app.wrapresponse(data));
                }
            });
            ...
            io._messagesRef.on('child_changed', function (childSnapshot, prevChildName) {
                if (childSnapshot.val() && childSnapshot.val() != undefined) {
                    ...
                    app.io.of('/messages').emit('child_changed', app.wrapresponse(data));
                }
            });
