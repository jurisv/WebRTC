    ...
    var firebase = require('./../firebase.db.js')(nconf),
        ...
        securityroles = {
            ...
            create: function(config,callback){
                ...
                if(records instanceof Array){
                    records.forEach(function(item) {
                        firebase.child('securityroles/' + item.id).update(item, function(err){
                            if(err){
                                callback(err,null);
                            }else{
                    ...
                    firebase.child('securityroles/' + config.records.id).update(config.records, function(err){
                        if(err){
                            callback(err,null);
                        }else{
            ...
            delete: function(config,callback){
                ...
                if(records instanceof Array){
                    records.forEach(function(item) {
                        firebase.child('securityroles/' + item.id).remove(function(err){
                            if(err){
                                callback(err,null);
                            }else{
                                callback(null,item);
                    ...
                    if (!!config.records.id) {
                        firebase.child('securityroles/' + config.records.id).remove(function (err) {
                            if (err) {
                                callback(err, null);
                            } else {
